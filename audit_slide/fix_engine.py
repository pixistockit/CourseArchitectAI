...
try:
    from audit_slide import config as CFG
except ImportError:
    class CFG:
...
def load_fixer_config():
    try:
        ...
        llm_config_path = os.path.join(os.path.dirname(__file__), '../../data/config/llm_config.json')
        merged_config = {}
...
class FixEngine:
    def __init__(self):
        ...
        sys_min = getattr(CFG, 'BODY_FONT_SIZE_MIN', 25)
    ...
    def apply_fixes(self, source_pptx, fixes_payload, output_dir):
        logger.info(f"🚀 STARTING FIX ENGINE")
        try: prs = Presentation(source_pptx)
        except Exception as e: raise ValueError(f"Could not open PPTX: {e}")
        ...
        for fix in fixes_payload:
            try:
                ...
                if slide_num < 1 or slide_num > len(prs.slides): continue
                ...
                logger.error(err); self.log_report.append(err)

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        new_filename = f"{os.path.splitext(os.path.basename(source_pptx))[0]}_REMEDIATED_{timestamp}.pptx"
        output_path = os.path.join(output_dir, new_filename)
        prs.save(output_path)
        log_path = os.path.join(output_dir, f"fix_log_{timestamp}.json")
        with open(log_path, 'w') as f: json.dump(self.log_report, f, indent=4)
        return output_path

    # --- HELPERS FOR SMART CONTRAST ---
    ...
    def _determine_real_background(self, target_shp, slide):
        ...
        # 2. Check shapes behind it (Simple Z-order check based on index)
        # Note: FixEngine doesn't have advanced overlap logic, so we assume WHITE if transparent.
        # Ideally we would check overlap, but for "Safe Fix" we prioritize text on solid BG.
        return (255, 255, 255) # Default to White if we can't determine

    def _iter_all_text_frames(self, shapes):
        for shape in shapes:
            name = shape.name
            if shape.has_text_frame: yield shape.text_frame, name, shape 
            if shape.has_table:
                for row in shape.table.rows:
                    for cell in row.cells:
                        if cell.text_frame: yield cell.text_frame, f"{name} (Table Cell)", shape
    ...
    def _apply_safe_rule(self, slide, rule_name, brand_rules, slide_num, target_shape_name=None):
        ...
        def should_process(current_shape_name):
            ...
            return target_shape_name.strip() == current_shape_name.strip()
        ...
        if rule_name == 'fix_font_family':
                    ...
                    for p in slide.notes_slide.notes_text_frame.paragraphs:
                        for run in p.runs:
                            if run.font.name != self.font_notes:
                                run.font.name = self.font_notes; made_change = True
                    if made_change: self.log_report.append(f"Slide {slide_num} (Notes): Standardized font to {self.font_notes}")
                return made_change
            for text_frame, shape_name, shape_obj in self._iter_all_text_frames(slide.shapes):
                ...
                if shape_obj.is_placeholder:
                    ...
                    if ph_type in (PP_PLACEHOLDER.TITLE, PP_PLACEHOLDER.CENTER_TITLE): target_font = self.font_title
                for p in text_frame.paragraphs:
                    for run in p.runs:
                        if run.font.name != target_font:
                            run.font.name = target_font; made_change = True
                if made_change: self.log_report.append(f"Slide {slide_num} ({shape_name}): Standardized font to {target_font}")
                ...
                for paragraph in text_frame.paragraphs:
                    for run in paragraph.runs:
                        if run.font.size is not None and run.font.size < target_min:
                            old_sz = run.font.size.pt; run.font.size = target_min
                            self.log_report.append(f"Slide {slide_num} ({shape_name}): Bumped font {old_sz}pt -> {self.min_font_size_pt}pt"); made_change = True
                        elif len(run.text.strip()) > 0 and run.font.size is None:
                             run.font.size = target_min
                             self.log_report.append(f"Slide {slide_num} ({shape_name}): Enforced {self.min_font_size_pt}pt on unstyled text"); made_change = True
            ...
            if not slide.shapes.title: pass 
                ...
                self.log_report.append(f"Slide {slide_num}: Inserted placeholder title."); made_change = True

        elif rule_name == 'fix_contrast':
            target_ratio = getattr(CFG, 'WCAG_RATIO_NORMAL', 4.5)
            for text_frame, shape_name, shape_obj in self._iter_all_text_frames(slide.shapes):
                if not should_process(shape_name): continue
                # DETECT REAL BACKGROUND (Context Aware)
                bg_rgb = self._determine_real_background(shape_obj, slide)
                
                for paragraph in text_frame.paragraphs:
                    for run in paragraph.runs:
                        current_rgb = (0, 0, 0)
                        if run.font.color and run.font.color.type == MSO_SHAPE_TYPE.AUTO: current_rgb = (0,0,0)
                        elif run.font.color and run.font.color.rgb: current_rgb = (run.font.color.rgb[0], run.font.color.rgb[1], run.font.color.rgb[2])
    ...
    def _get_wcag_passing_color(self, fg_rgb, bg_rgb, target_ratio):
        def get_luminance(rgb):
            rgb_linear = []
            for c in rgb:
                c = c / 255.0; val = c / 12.92 if c <= 0.03928 else ((c + 0.055) / 1.055) ** 2.4
                rgb_linear.append(val)
            return 0.2126 * rgb_linear[0] + 0.7152 * rgb_linear[1] + 0.0722 * rgb_linear[2]
        def get_contrast(rgb1, rgb2):
            lum1 = get_luminance(rgb1); lum2 = get_luminance(rgb2)
            return (max(lum1, lum2) + 0.05) / (min(lum1, lum2) + 0.05)
        current_fg = list(fg_rgb)
        if get_contrast(current_fg, bg_rgb) >= target_ratio: return tuple(current_fg)
        bg_lum = get_luminance(bg_rgb)
        is_light_bg = bg_lum > 0.5
        for _ in range(20):
            contrast = get_contrast(current_fg, bg_rgb)
            if contrast >= target_ratio: return tuple(current_fg)
            current_fg = [max(0, int(c * 0.90)) for c in current_fg] if is_light_bg else [min(255, int(c + (255-c)*0.10)) for c in current_fg]
        return (0,0,0) if is_light_bg else (255,255,255)

    # ... (Keep existing helpers: _get_rules_for_slide, _find_best_text_shape, _capture_layout_map, _replace_text_enforced) ...
    def _get_rules_for_slide(self, slide):
        rules = {"body_font": CFG.BODY_FONT_NAME, "title_font": CFG.TITLE_FONT_NAME, "notes_font": CFG.NOTES_FONT_NAME, "safe_color": RGBColor(*CFG.NOTES_FONT_COLOR_RGB)}
        try:
            master = slide.slide_layout.slide_master
            if master.text_styles and master.text_styles.body_style:
                font = master.text_styles.body_style.levels[0].font
                if font.name: rules['body_font'] = font.name
            if master.text_styles and master.text_styles.title_style:
                font = master.text_styles.title_style.levels[0].font
                if font.name: rules['title_font'] = font.name
        except: pass
        if self.user_config:
    ...
    def _find_best_text_shape(self, slide):
        ...
        best_shape = None; max_words = 0
        for shape in iter_shapes(slide.shapes):
            if shape.has_text_frame and shape.text.strip():
                word_count = len(shape.text.split())
                if word_count > max_words and word_count > 5: max_words = word_count; best_shape = shape
        return best_shape
        ...
        try:
            for p in text_frame.paragraphs:
                ...
                if lvl not in layout_map:
                    layout_map[lvl] = {"space_before": p.space_before, "space_after": p.space_after, "line_spacing": p.line_spacing, "alignment": p.alignment}
        except: pass
        return layout_map

    def _replace_text_enforced(self, text_frame, new_content, layout_map, target_type, brand_rules):
        text_frame.clear(); lines = new_content.split('\n')
        enforced_font = brand_rules["notes_font"] if target_type == 'notes' else brand_rules["body_font"]
        for line in lines:
            stripped = line.strip()
            if not stripped: continue
            p = text_frame.add_paragraph(); level = 0; clean_text = stripped
            if line.startswith('    ') or line.startswith('\t') or stripped.startswith(('o ', '- ', '– ')): level = 1; clean_text = re.sub(r'^[\s\t]*[o\-–]\s*', '', line).strip()
            elif stripped.startswith(('* ', '• ')): level = 0; clean_text = stripped[2:].strip()
            elif re.match(r'^\d+\.', stripped): level = 0; clean_text = stripped
            p.level = level; p.text = clean_text
            ...
            if p.runs:
                run = p.runs[0]; run.font.name = enforced_font 
                if not run.font.color or not run.font.color.rgb: run.font.color.rgb = brand_rules["safe_color"]